...
1. **Group Isomorphism**
   ...
   - **Steps to prove isomorphism**
     - **Step 1: "Mapping"**
       - Define a candidate function $\phi$ from $G$ to $G'$.
       - This function $\phi$ should be thoughtfully chosen to potentially satisfy the properties of an isomorphism (bijective and operation-preserving).
     - **Step 2: "1-1" (One-to-One)**
       - Prove that $\phi$ is injective (one-to-one).
       - To do this, assume $\phi(a) = \phi(b)$ for some elements $a, b \in G$ and demonstrate that this implies $a = b$.
     - **Step 3: "Onto"**
       - Prove that $\phi$ is surjective (onto).
       - This means that for any element $g' \in G'$, there exists an element $g \in G$ such that $\phi(g) = G'$.
     - **Step 4: "O.P." (Operation-Preserving)**
       - Prove that $\phi$ preserves the group operation.
       - Specifically, show that for all $a, b \in G$,
          $$
          \phi(ab) = \phi(a)\phi(b).
          $$

2. **Automorphism**: An automorphism is an isomorphism from a group $G$ onto itself. The set of all automorphisms of $G$ is denoted $\text{Aut}(G)$.
...
5. **Order Preservation**:
   - **Statement**: $\vert a\vert  = \vert \varphi(a)\vert$ for all $a \in G$.
   - **Proof**:
     - Let $\vert a\vert  = m$, so $a^m = e$, where $e$ is the identity element in $G$.
     ...
     - Since $\varphi$ is one-to-one, $a$ and $\varphi(a)$ must have the same order, thus $\vert a\vert  = \vert \varphi(a)\vert$.
