...
Because $x \neq-6$ and $y \neq -6$, we have $90(x+6)(y+6) -6 \neq -6$, so $x * y  \in\mathbb{Q}\setminus\left\{ -6 \right\}$.
...
#### Problem 1.2
> Let $(\text{G},*)$ be a group. Assume $a \in \text{G}$. $\forall x,y \in \text{G}$, put $x \# y=x*a*y$,
> Show that $(\text{G}, \#)$ is a group.

- **First, we show that $\#$ is a binary operation on $\text{G}$.**
Since $x, a, y \in \text{G}$ and $\text{G}$ is closed under $*$, we have $x \# y = x * a * y \in \text{G}$.
...
- **Next, we find the identity element $e' \in \text{G}$ such that $x \# e' = x$ and $e' \# x = x$.**
We need:
$$
\begin{align}
x \# e' &= x \\
...
- **Next, we show that every element $x \in \text{G}$ has an inverse $x^{\prime} \in \text{G}$ such that $x \# x^{\prime} = e'$.**
...
Let $A = \begin{pmatrix} x_1 & y_1 \\ 2 y_1 & x_1 \end{pmatrix}$ and $B = \begin{pmatrix} x_2 & y_2 \\ 2 y_2 & x_2 \end{pmatrix}$ in $\text{G}$. Compute $A B$:
$$
A B = \begin{pmatrix}
